...
public class SessionDBContext extends DBContext {
    ...
    public ArrayList<Session> getSessionsByUserID(Integer userId, Date date) {
        String sql = "select distinct [s].*,[r].[name] as [rname],\n" +
"                                                [c].[name] as [cname], \n" +
"                                                [co].[name] as [coname],\n" +
"                                                [a].[status] as [astatus]\n" +
"                                                from user_class as [e], [session] as [s], [room] as [r], [class] as [c], [course] as [co], attended as [a]\n" +
"                                                where s.[userid] = ?\n" +
...
"                                                and [a].roomid = [s].roomid\n" +
"                								and [a].slotid = [s].slotid\n" +
"                								and [a].date = [s].date";
        ...
        try ( PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                Session session = new Session();
                session.setSlot(rs.getInt("slotid"));
                session.setDate(rs.getDate("date"));
                Room room = new Room();
